...
<xsl:stylesheet version="1.0"
  ...
  <xsl:variable name="mode" select="'予実'"/>
  <xsl:template match="/">
    <html>
      <head>
        <title>プロジェクト</title>
      ...
      <tr>
        <td align="center">
          <small>
            <br/>
            <B>
              <xsl:value-of select="'すべてのプロジェクト'"/>
            </B>
            <br/>
          </small>
        </td>
  ...
  <xsl:template match="全体">
    ...
    <table class="disp" cellpadding="0" cellspacing="0" width="100%">
        ...
        <xsl:choose>
          <xsl:when test="$disp=0">
            <xsl:call-template name="FOOT">
              <xsl:with-param name="project" select="プロジェクト[原価率[@mode=$mode] != '']" />
            </xsl:call-template>
          </xsl:when>
          <xsl:when test="$disp=1">
            <xsl:call-template name="FOOT">
          ...
          <xsl:when test="$disp=2">
            <xsl:call-template name="FOOT">
              <xsl:with-param name="project" select="プロジェクト[原価[@mode=$mode] = 0]" />
            </xsl:call-template>
          </xsl:when>
          <xsl:otherwise>
            <xsl:call-template name="FOOT">
      ...
      <tbody>
        <xsl:choose>
          <xsl:when test="$disp=0">
            <xsl:for-each select="プロジェクト[原価率[@mode=$mode] != '']">
          ...
          <xsl:when test="$disp=2">
            <xsl:for-each select="プロジェクト[原価[@mode=$mode] = 0]">
              <xsl:sort select="原価率[@mode=$mode]" data-type="number" order="descending"/>
              <xsl:apply-templates select=".">
                <xsl:with-param name="pos" select="position()" />
              </xsl:apply-templates>
          ...
          <xsl:otherwise>
            <xsl:for-each select="プロジェクト">
              <xsl:sort select="原価率" data-type="number" order="descending"/>
              <xsl:apply-templates select=".">
                <xsl:with-param name="pos" select="position()" />
              </xsl:apply-templates>
            </xsl:for-each>
  ...
  <xsl:template name="HEAD">
    <tr>
      ...
      <!--<th>
        ...
        </xsl:attribute>        <xsl:value-of select="'状況'"/>
      </th>-->
    </tr>
  </xsl:template>
  ...
  <xsl:template name="FOOT">
    ...
    <tr>
      <td class="pItem" colspan="8" style="text-align:right">
        <xsl:value-of select="'合計'"/>
      </td>
      <td class="R_Foot">
        <!-- 売上 -->
  ...
  <xsl:template match="pCode">
    ...
    <xsl:attribute name="memo">
      ...
      <xsl:value-of select="../合計工数[@mode='実績']"/>
      <xsl:value-of select="'  '"/>
      <xsl:value-of select="../状態"/>
    </xsl:attribute>
    <xsl:choose>
      <xsl:when test="../状態 = 0 and ../合計工数[@mode='実績'] > 0">
        <xsl:attribute name="style">
    ...
    <table class="disp">
      ...
      <tbody>
        <tr>
          ...
          <td class="l_Item">
            <xsl:value-of select="'[状態]が[引合中]で[実績工数]が発生'"/>
          </td>
        </tr>
      </tbody>
    </table>
