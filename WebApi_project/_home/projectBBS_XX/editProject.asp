		...
		<TABLE ALIGN="center" border="0" cellPadding="2" cellSpacing="0" WIDTH="80%">
			...
			<TBODY>
				...
				<TR>
					...
					<TD><INPUT class='textFont' ID="T1" size='45' maxlength="30" style='width:99%'></TD>
				</TR>
				<TR>
					<TD nowrap class='textName'>客先担当者</TD>
					<TD><INPUT class='textFont' ID="T2" size='45' maxlength="20" style='width:99%'></TD>
				</TR>
				...
				<TR>
					...
					<TD colspan="2"><TEXTAREA class='textFont' ID="Content" rows="10" cols="50" WIDTH="100%" style="font-family:arial"></TEXTAREA></TD>
				</TR>
			</TBODY>
		</TABLE>
</BODY>
...
var CategoryTab1 = ""
var CategoryTab2 = ""
var parent_Buff = new Object
var childlen_ReturnValue = ""
...
function inputCorp()
   ...
   //windowOption += "status:no;scroll:no;"
	...
	var width = 300
	var height = 150
	parent_Buff["VALUE"] = corpSEL.newValue
	dispInputText(URL, para, title, width, height)
...
function inputCategory(categoryNo)
   {
   //var Buff = new Object
	//Buff["TITLE"] = "カテゴリー名設定"
   ...
   //windowOption += "status:no;scroll:no;"
	...
	var TimeStamp = String(parseInt(Date.UTC(tm)/1000000))
	var para = "?TimeStamp=" + TimeStamp
	var title = "カテゴリー名" + categoryNo + "設定"
	var width = 300
	var height = 150
	switch (categoryNo) {
		case 1:
			parent_Buff["VALUE"] = categorySEL1.newValue
			break;
		case 2:
			parent_Buff["VALUE"] = categorySEL2.newValue
			break;
	}
	dispInputText(URL, para, title, width, height)
...
<SCRIPT LANGUAGE="JavaScript">
	$(document).on('change', '#salesManSEL', function() {
		var selPos = this.selectedIndex
		T3.value   = this.options[selPos].text
	});

	$(document).on('change', '#kindSEL', function() {
		var selPos = this.selectedIndex
		T_kind.value   = this.options[selPos].value
	});

	$(document).on('change', '#kindNewSEL', function() {
		var selPos = this.selectedIndex
		T_kindNew.value   = this.options[selPos].value
	});

	$(document).on('change', '#fixLevelSEL', function() {
		var selPos = this.selectedIndex
...
function InitData()
    ...
    Content.value = Content.value.replace(/<br\/>/g, "\n");
