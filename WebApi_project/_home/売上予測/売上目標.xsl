  ...
  <xsl:template name="グループ">
	...
	<xsl:template name="item_Loop">
		...
		<xsl:if test="$cnt &lt; $max">
			<td class="num">
				...
				<xsl:value-of select="format-number( sum($data/月[@m=$cnt]) div 1000 ,$form)"/>
			</td>
			<xsl:call-template name="item_Loop">
	...
	<xsl:template name="item_Loop_累計">
		...
		<xsl:if test="$cnt &lt; $max">
			<td class="num">
				<xsl:attribute name="nowrap"/>
				<xsl:value-of select="format-number( ( $work + sum($data/月[@m=$cnt])) div 1000 ,'#,##0')"/>
			</td>
			<xsl:call-template name="item_Loop_累計">
				<xsl:with-param name="data" select="$data" />
				<xsl:with-param name="max" select="$max" />
	...
	<xsl:template name="item_Loop_差">
		...
		<xsl:if test="$cnt &lt; $max">
			<xsl:variable name="work" select="($work_B + sum($data_B/月[@m=$cnt])) - ($work_A + sum($data_A/月[@m=$cnt]))"/>
	...
	<xsl:template name="item_Loop_率">
		...
		<xsl:param name="work_B" select="0"/>
		<xsl:if test="$cnt &lt; $max">
			<xsl:variable name="work" select="($work_B + sum($data_B/月[@m=$cnt])) div ($work_A + sum($data_A/月[@m=$cnt]))"/>
			<td class="num">
				<xsl:attribute name="nowrap"/>
				<xsl:choose>
