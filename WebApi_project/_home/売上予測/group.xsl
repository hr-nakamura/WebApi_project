...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:template match="/">
    ...
    <xsl:if test="count(*) > 0">
      ...
      <xsl:for-each select="*">
            ...
            <xsl:call-template name="Section">
              <xsl:with-param name="部門名" select="*"/>
            </xsl:call-template>
          </xsl:for-each>
    ...
    <xsl:for-each select="$部門名[count(uName/*) &gt; 0]">
    ...
    <tr class="sub_line">
      ...
      <td class="num">
        <xsl:value-of select="format-number( sum($部門名/uName/*/data/月) ,$form)"/>
      </td>
      <xsl:call-template name="合計">
        <xsl:with-param name="data" select="$部門名/uName/*/data/月"/>
        <xsl:with-param name="begin" select="0"/>
        <xsl:with-param name="max" select="12"/>
      </xsl:call-template>
      <td class="num">
