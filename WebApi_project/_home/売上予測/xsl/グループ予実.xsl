...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
  ...
  <xsl:template match="/">
	...
	<xsl:template name="item_Loop2">
		...
		<xsl:if test="$cnt &lt; $max">
      <xsl:variable name="temp_予測" select="sum($data/予測/月[@m=$cnt])"/>
      <xsl:variable name="temp_実績" select="sum($data/実績/月[@m=$cnt])"/>
      <xsl:variable name="temp" select="($temp_実績 div $temp_予測)"/>
      <td class="num">
				<xsl:attribute name="nowrap"/>
				<xsl:if test="$temp &gt; 0">
					<xsl:value-of select="format-number( $temp ,$form)"/>
    ...
    <xsl:if test="$cnt &lt; $max">
      <xsl:variable name="temp" select="sum($data/月[@m=$cnt])"/>
      <td class="num" style="font-color:red;">
		  <xsl:if test="$mode ='予測'">
    		  <xsl:attribute name="class">
	    		  <xsl:value-of select="'yosoku'"/>
		      </xsl:attribute>
